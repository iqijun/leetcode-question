...
public class MaximumAlternatingSubsequenceSum{
  public static void main(String[] args) {
      ...
      solution.maxAlternatingSum(new int[]{6,2,1});
  }
...
class Solution {
    public long maxAlternatingSum(int[] nums) {
        ...
        long maxOddSum = 0;
        for (int i = 0; i < n; i++) {
            maxEvenSum = Math.max(maxOddSum + nums[i], maxEvenSum);
            maxOddSum = Math.max(maxEvenSum - nums[i], maxOddSum);
        }
        return Math.max(maxOddSum, maxEvenSum);
    }
//    public long maxAlternatingSum(int[] nums) {
